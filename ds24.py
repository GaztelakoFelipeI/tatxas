...
while cap.isOpened():
    ...
    # --- INICIO: NMS ADICIONAL SOBRE DETECCIONES FILTRADAS (ANTES DEL TRACKING) ---
    if detecciones_actuales: # Solo si hay detecciones para procesar
        # Ordenar por 'cx' puede ayudar un poco, pero no es estrictamente necesario para la correctitud
        # detecciones_actuales_sorted = sorted(detecciones_actuales, key=lambda d: d['cx'])
        # Usaremos la lista directamente para simplificar, la eficiencia no será un gran problema con pocas detecciones
        ...
        for i in range(len(detecciones_actuales)):
            ...
            # Conservar la detección i temporalmente
            current_detection = detecciones_actuales[i]
            is_current_suppressed = False
            ...
            for j in range(i + 1, len(detecciones_actuales)):
                ...
                if dist < NMS_PRETRACK_DIST_THRESHOLD:
                    # Si la detección 'j' está demasiado cerca de 'current_detection',
                    # suprimir 'j'. Podrías añadir lógica para decidir cuál mantener
                    # (ej. basada en confianza si YOLO la da, o si una está "más al centro" del ROI, etc.)
                    # Por ahora, simplemente suprimimos 'j'.
                    # Si quisiéramos suprimir 'i' en lugar de 'j' bajo alguna condición,
                    # se marcaría is_current_suppressed = True y se rompería el bucle interno.
                    suppressed_indices.add(j)

            if not is_current_suppressed: # Si la detección actual no fue suprimida por una "mejor"
                final_detections_after_nms.append(current_detection)

        # Opcional: Imprimir cuántas detecciones se eliminaron
        # num_removed = len(detecciones_actuales) - len(final_detections_after_nms)
        # if num_removed > 0:
        #     print(f"NMS Pre-Track: Removidas {num_removed} detecciones redundantes.")
    ...
    # for tracker in active_trackers:
    #     if tracker.estado == "Tentativo" and tracker.hits >= MIN_HITS_TO_CONFIRM:
    #         is_too_close_to_recent_confirmed = False
    #         current_cx_img_tentative = tracker.centro_img[0]
    #         current_cy_img_tentative = tracker.centro_img[1]

    #         for recent_confirmed_tacha in ultimas_5_ids_confirmadas:
    #             dist_to_recent = np.linalg.norm([current_cx_img_tentative - recent_confirmed_tacha['cx'],
    #                                                current_cy_img_tentative - recent_confirmed_tacha['cy']])
    #             if dist_to_recent < PROXIMITY_THRESHOLD_FOR_ID_SUPPRESSION:
    #                 is_too_close_to_recent_confirmed = True
    #                 break

    #         if not is_too_close_to_recent_confirmed:
    #             tracker.estado = "Confirmado"
    #             if tracker.display_id is None:
    #                 tracker.display_id = next_sequential_confirmed_id
    #                 next_sequential_confirmed_id +=1
    ...
    for tracker_candidate in active_trackers:
        if tracker_candidate.estado == 'Tentativo' and tracker_candidate.hits >= MIN_HITS_TO_CONFIRM:
            is_too_close = False
            cx_candidate = tracker_candidate.centro_img[0]
            cy_candidate = tracker_candidate.centro_img[1]

            # A. Chequear contra trackers YA confirmados (de frames/iteraciones anteriores)
            for other_tracker in active_trackers:
                if other_tracker.id == tracker_candidate.id:
                    continue # No compararse consigo mismo
                
                if other_tracker.estado == 'Confirmado' and other_tracker.display_id is not None:
                    dist = np.linalg.norm([cx_candidate - other_tracker.centro_img[0],
                                           cy_candidate - other_tracker.centro_img[1]])
                    if dist < PROXIMITY_THRESHOLD_FOR_ID_SUPPRESSION:
                        is_too_close = True
                        print(f"ID INTERNA {tracker_candidate.id} (tentativo) demasiado cerca de ID CONFIRMADA {other_tracker.display_id}. No se confirma.")
                        break
            
            if is_too_close:
                continue 

            # B. Chequear contra trackers que se acaban de confirmar EN ESTA MISMA PASADA DEL FRAME
            for just_confirmed_tracker in newly_confirmed_this_frame_pass:
                # No es necesario chequear ID aquí, son diferentes objetos tracker en la lista de candidatos
                dist = np.linalg.norm([cx_candidate - just_confirmed_tracker.centro_img[0],
                                       cy_candidate - just_confirmed_tracker.centro_img[1]])
                if dist < PROXIMITY_THRESHOLD_FOR_ID_SUPPRESSION:
                    is_too_close = True
                    # print(f"ID INTERNA {tracker_candidate.id} (tentativo) demasiado cerca de ID RECIÉN CONFIRMADA EN ESTE FRAME {just_confirmed_tracker.display_id}. No se confirma.")
                    break
            
            if not is_too_close:
                tracker_candidate.estado = 'Confirmado'
                if tracker_candidate.display_id is None: # Debería ser None si viene de Tentativo
                    tracker_candidate.display_id = next_sequential_confirmed_id
                    next_sequential_confirmed_id += 1

                    # Añadir a la lista de recién confirmados en esta pasada para que los siguientes candidatos lo vean
                    newly_confirmed_this_frame_pass.append(tracker_candidate)

                    # Lógica original para actualizar estadísticas de frames, historial_confirmadas, ultimas_5_ids_confirmadas
                    if not detecciones_frames or (frame_idx - detecciones_frames[-1]) >= min_frames_entre_detecciones:
                        detecciones_frames.append(frame_idx)
                        if len(detecciones_frames) >= ventana_inicial + 1:
                            intervalos = [j - i for i, j in zip(detecciones_frames[:-1], detecciones_frames[1:])]
                    ...
                    historial_confirmadas.append({
                        'internal_id': tracker_candidate.id,
                        'tacha_id_secuencial': tracker_candidate.display_id,
                        'centro_transformado': tracker_candidate.centro_transformado.copy(),
                        'centro_img': tracker_candidate.centro_img.copy(),
                        'frame': tracker_candidate.frame_idx,
                        'clase': 'tacha'
                    })
                    
                    new_confirmed_info_dict = { # Renombrado para evitar conflicto con la variable externa
                        "id": tracker_candidate.display_id,
                        "cx": int(tracker_candidate.centro_img[0]),
                        "cy": int(tracker_candidate.centro_img[1])
                    }
                    ultimas_5_ids_confirmadas.append(new_confirmed_info_dict)
                    if len(ultimas_5_ids_confirmadas) > MAX_DISPLAY_IDS:
                        ultimas_5_ids_confirmadas.pop(0)
            # else: (Si is_too_close es True después del segundo chequeo)
                # print(f"Tracker {tracker_candidate.id} no se confirma por proximidad a uno recién confirmado en este frame.")
                # No se hace nada, permanecerá 'Tentativo' y se gestionará por misses.

    # Filtrar trackers finales basados en misses (después de que todas las confirmaciones/supresiones del frame se hayan decidido)
    final_trackers = []
    for tracker in active_trackers:
        if tracker.misses < MAX_MISSES_TO_DELETE:
            final_trackers.append(tracker)
        else:
            # Este print es genérico. Si quieres saber si fue suprimido por proximidad y luego eliminado por misses,
            # necesitarías una bandera adicional en el objeto tracker o una lógica más compleja aquí.
            print(f"Eliminando tracker ID {tracker.id}, ID secuencial {tracker.display_id} por exceso de misses.")
    active_trackers = final_trackers
    ...
    # mini_mapa = np.ones((mini_mapa_h, mini_mapa_w, 3), dtype=np.uint8) * 255 # Original
    mini_mapa_bg_color = (220, 220, 220) # Gris claro para el fondo
    mini_mapa = np.full((mini_mapa_h, mini_mapa_w, 3), mini_mapa_bg_color, dtype=np.uint8)

    escala_mapa_vo = 5  # pixeles por unidad de t_f (asumamos metros)
    centro_mapa_x, centro_mapa_y = mini_mapa_w // 2, mini_mapa_h // 2

    grid_color = (180, 180, 180) # Color de la grilla
    axes_color = (0, 0, 0)       # Color de los ejes
    path_color_vo = (0, 0, 255)  # Rojo para la trayectoria VO
    current_pos_color = (0, 255, 0) # Verde para la posición actual
    arrow_color = (255, 0, 0)   # Azul para la flecha de dirección
    text_color = (0, 0, 0)      # Negro para el texto de la escala

    # Dibuja la Grilla y Ejes
    grid_spacing_pixels = 25 # Cada 25 pixeles una línea de grilla
    meters_per_grid = grid_spacing_pixels / escala_mapa_vo

    # Líneas de la grilla vertical
    for x in range(0, mini_mapa_w, grid_spacing_pixels):
        cv2.line(mini_mapa, (x, 0), (x, mini_mapa_h), grid_color, 1)
    # Líneas de la grilla horizontal
    for y in range(0, mini_mapa_h, grid_spacing_pixels):
        cv2.line(mini_mapa, (0, y), (mini_mapa_w, y), grid_color, 1)

    # Ejes principales
    cv2.line(mini_mapa, (0, centro_mapa_y), (mini_mapa_w, centro_mapa_y), axes_color, 1) # Eje X
    cv2.line(mini_mapa, (centro_mapa_x, 0), (centro_mapa_x, mini_mapa_h), axes_color, 1) # Eje Z

    # Escalas Visuales (Ticks y Texto)
    scale_font_scale = 0.3
    scale_font_thickness = 1
    # Eje X (horizontal en el mapa, corresponde a t_f[0])
    for x_pix in range(centro_mapa_x, mini_mapa_w, grid_spacing_pixels): # Positivo
        dist_m = (x_pix - centro_mapa_x) / escala_mapa_vo
        if dist_m > 0: # No dibujar en el origen dos veces
    ...
    for x_pix in range(centro_mapa_x - grid_spacing_pixels, 0, -grid_spacing_pixels): # Negativo
        dist_m = (x_pix - centro_mapa_x) / escala_mapa_vo
        cv2.line(mini_mapa, (x_pix, centro_mapa_y - 3), (x_pix, centro_mapa_y + 3), axes_color, 1)
        cv2.putText(mini_mapa, f"{dist_m:.0f}m", (x_pix + 2, centro_mapa_y - 5), cv2.FONT_HERSHEY_SIMPLEX, scale_font_scale, text_color, scale_font_thickness)

    # Eje Z (vertical en el mapa, corresponde a t_f[2])
    # Nota: En el mapa, Z positivo suele ir hacia abajo desde el centro.
    # La odometría visual tiene t_f[2] positivo "hacia adelante".
    # Si la trayectoria se dibuja con Z positivo hacia abajo en el mapa:
    for y_pix in range(centro_mapa_y + grid_spacing_pixels, mini_mapa_h, grid_spacing_pixels): # Positivo Z (hacia abajo en mapa)
        dist_m = (y_pix - centro_mapa_y) / escala_mapa_vo
    ...
    for y_pix in range(centro_mapa_y - grid_spacing_pixels, 0, -grid_spacing_pixels): # Negativo Z (hacia arriba en mapa)
        dist_m = (y_pix - centro_mapa_y) / escala_mapa_vo
        cv2.line(mini_mapa, (centro_mapa_x - 3, y_pix), (centro_mapa_x + 3, y_pix), axes_color, 1)
        cv2.putText(mini_mapa, f"{dist_m:.0f}m", (centro_mapa_x + 5, y_pix + 3), cv2.FONT_HERSHEY_SIMPLEX, scale_font_scale, text_color, scale_font_thickness)
    ...
    # Dibuja la trayectoria de Odometría Visual
    if len(posiciones_vehiculo) > 1:
        for i in range(1, len(posiciones_vehiculo)):
            # Las posiciones son [x, z]
            x1_vo, z1_vo = posiciones_vehiculo[i-1]
            x2_vo, z2_vo = posiciones_vehiculo[i]
            # Convertir a coordenadas del mapa: X_mapa = X_vo * escala + centro_X, Z_mapa = Z_vo * escala + centro_Z
            # El eje Z de la odometría es positivo hacia adelante. En el mapa, esto puede ser hacia abajo o arriba.
            # Si Z positivo es hacia abajo en el mapa (típico en imágenes):
            ...
            # Asegurarse que los puntos están dentro de los límites del minimapa antes de dibujar
            if (0 <= p1_mapa[0] < mini_mapa_w and 0 <= p1_mapa[1] < mini_mapa_h and
                0 <= p2_mapa[0] < mini_mapa_w and 0 <= p2_mapa[1] < mini_mapa_h):
                cv2.line(mini_mapa, p1_mapa, p2_mapa, path_color_vo, 1) # Línea más delgada para la trayectoria

    # Dibuja el punto de posición actual
    if posiciones_vehiculo:
        x_curr_vo, z_curr_vo = posiciones_vehiculo[-1]
        current_pos_on_map = (int(x_curr_vo * escala_mapa_vo + centro_mapa_x),
                                int(z_curr_vo * escala_mapa_vo + centro_mapa_y))
        if (0 <= current_pos_on_map[0] < mini_mapa_w and 0 <= current_pos_on_map[1] < mini_mapa_h):
            cv2.circle(mini_mapa, current_pos_on_map, 3, current_pos_color, -1) # Círculo relleno

        # Dibuja la Flecha de Orientación
        if len(posiciones_vehiculo) >= 2:
            x_prev_vo, z_prev_vo = posiciones_vehiculo[-2]
            prev_pos_on_map = (int(x_prev_vo * escala_mapa_vo + centro_mapa_x),
                                int(z_prev_vo * escala_mapa_vo + centro_mapa_y))

            # Solo dibujar flecha si hay movimiento significativo para evitar ruido
            if abs(current_pos_on_map[0] - prev_pos_on_map[0]) > 0 or abs(current_pos_on_map[1] - prev_pos_on_map[1]) > 0:
                # La flecha comienza en la posición anterior y apunta hacia la actual,
                # luego la extendemos un poco para que represente la dirección desde la posición actual.
                dx = current_pos_on_map[0] - prev_pos_on_map[0]
                dy = current_pos_on_map[1] - prev_pos_on_map[1]
                ...
                if magnitude > 0: # Evitar división por cero
                    ...
                    arrow_length = 15 # Longitud de la flecha en píxeles
                    arrow_tip_x = int(current_pos_on_map[0] + udx * arrow_length)
                    arrow_tip_y = int(current_pos_on_map[1] + udy * arrow_length)

                    # Asegurar que la punta de la flecha también esté dentro de los límites si es posible
                    # (cv2.arrowedLine podría manejar el clipping)
                    # arrow_tip_x = np.clip(arrow_tip_x, 0, mini_mapa_w - 1)
                    # arrow_tip_y = np.clip(arrow_tip_y, 0, mini_mapa_h - 1)

                    if (0 <= current_pos_on_map[0] < mini_mapa_w and 0 <= current_pos_on_map[1] < mini_mapa_h and
                        0 <= arrow_tip_x < mini_mapa_w and 0 <= arrow_tip_y < mini_mapa_h):
                        cv2.arrowedLine(mini_mapa, current_pos_on_map, (arrow_tip_x, arrow_tip_y),
                                        arrow_color, 2, tipLength=0.4)

    # Coloca el mini-mapa en el frame principal
    # (Asegúrate que frame_corregido existe y tiene las dimensiones adecuadas)
    if frame_corregido.shape[0] >= mini_mapa_h + 10 and frame_corregido.shape[1] >= mini_mapa_w + 10:
        ...
        print("Advertencia: El frame_corregido es demasiado pequeño para el mini-mapa en la posición designada.")

    # --- INICIO: NUEVO DISEÑO DE VISUALIZACIÓN DE INFORMACIÓN ---
    font = cv2.FONT_HERSHEY_SIMPLEX
    line_type = cv2.LINE_AA

    # --- Colores y Tamaños de Fuente (AUMENTADOS) ---
    font_scale_top = 0.8
    font_scale_info_boxes = 0.7
    font_scale_bottom_center = 0.8
    font_color_general = (255, 255, 255)
    font_color_yellow = (0, 255, 255)  # Amarillo BGR
    thickness_general = 2

    # --- DEFINICIÓN DE TEXTOS PARA PILEN ---
    video_name_text = f"Video analizado: {os.path.basename(ruta_video)}"
    pos_text = f"Posicion: X: {t_f[0][0]:.1f}, Y: {t_f[2][0]:.1f}"
    code_text = f"Codigo: {script_name}"

    # --- INFORMACIÓN SUPERIOR CENTRADA EN PILEN ---
    (text_w_video, text_h_video), _ = cv2.getTextSize(video_name_text, font, font_scale_top, thickness_general)
    (text_w_pos, text_h_pos), _ = cv2.getTextSize(pos_text, font, font_scale_top, thickness_general)
    (text_w_code, text_h_code), _ = cv2.getTextSize(code_text, font, font_scale_top, thickness_general)

    center_x = int(frame_width / 2)
    start_y = 35
    line_spacing = 12  # Espacio entre líneas

    # 1. Nombre del video (arriba) - amarillo con borde negro
    y_video = start_y
    ...
    # 1. Nombre del video (arriba)
    y_video = start_y
    # "Video analizado:" amarillo, valor blanco
    x_video_label = center_x - (text_w_video_label + text_w_video_value + 5) // 2
    x_video_value = x_video_label + text_w_video_label + 5
    cv2.putText(frame_corregido, "Video analizado:", (x_video_label, y_video), font, font_scale_top, (0,0,0), thickness_general + 2, line_type)
    cv2.putText(frame_corregido, "Video analizado:", (x_video_label, y_video), font, font_scale_top, font_color_yellow, thickness_general, line_type)
    cv2.putText(frame_corregido, os.path.basename(ruta_video), (x_video_value, y_video), font, font_scale_top, (0,0,0), thickness_general + 2, line_type)
    cv2.putText(frame_corregido, os.path.basename(ruta_video), (x_video_value, y_video), font, font_scale_top, font_color_general, thickness_general, line_type)

    # 2. Posiciones (al medio) - amarillo el texto fijo, blanco los valores
    y_pos = y_video + text_h_video_label + line_spacing
    fixed_text = "Posicion:"
    value_text = f"X: {t_f[0][0]:.1f}, Y: {t_f[2][0]:.1f}"
    fixed_text_real = " | Posicion real:"
    if gps_posiciones_vehiculo:
        x_real, y_real = round(float(gps_posiciones_vehiculo[-1][1]), 2), round(float(gps_posiciones_vehiculo[-1][0]), 2)
    else:
        x_real, y_real = 0.0, 0.0  # Valor por defecto si no hay datos
    value_text_real = f"X: {x_real:.6f}, Y: {y_real:.6f}"

    (text_w_fixed, _), _ = cv2.getTextSize(fixed_text, font, font_scale_top, thickness_general)
    (text_w_value, _), _ = cv2.getTextSize(value_text, font, font_scale_top, thickness_general)
    (text_w_fixed_real, _), _ = cv2.getTextSize(fixed_text_real, font, font_scale_top, thickness_general)
    (text_w_value_real, _), _ = cv2.getTextSize(value_text_real, font, font_scale_top, thickness_general)

    total_width = text_w_fixed + text_w_value + text_w_fixed_real + text_w_value_real + 30
    x_fixed = center_x - total_width // 2
    x_value = x_fixed + text_w_fixed + 5
    x_fixed_real = x_value + text_w_value + 20
    x_value_real = x_fixed_real + text_w_fixed_real + 5

    # Amarillo para los textos fijos, blanco para los valores
    cv2.putText(frame_corregido, fixed_text, (x_fixed, y_pos), font, font_scale_top, (0,0,0), thickness_general + 2, line_type)
    cv2.putText(frame_corregido, fixed_text, (x_fixed, y_pos), font, font_scale_top, font_color_yellow, thickness_general, line_type)
    cv2.putText(frame_corregido, value_text, (x_value, y_pos), font, font_scale_top, (0,0,0), thickness_general + 2, line_type)
    cv2.putText(frame_corregido, value_text, (x_value, y_pos), font, font_scale_top, font_color_general, thickness_general, line_type)
    cv2.putText(frame_corregido, fixed_text_real, (x_fixed_real, y_pos), font, font_scale_top, (0,0,0), thickness_general + 2, line_type)
    cv2.putText(frame_corregido, fixed_text_real, (x_fixed_real, y_pos), font, font_scale_top, font_color_yellow, thickness_general, line_type)
    cv2.putText(frame_corregido, value_text_real, (x_value_real, y_pos), font, font_scale_top, (0,0,0), thickness_general + 2, line_type)
    cv2.putText(frame_corregido, value_text_real, (x_value_real, y_pos), font, font_scale_top, font_color_general, thickness_general, line_type)

    # 3. Código actual (abajo)
    y_code = y_pos + text_h_code_label + line_spacing
    x_code_label = center_x - (text_w_code_label + text_w_code_value + 5) // 2
    x_code_value = x_code_label + text_w_code_label + 5
    cv2.putText(frame_corregido, "Codigo:", (x_code_label, y_code), font, font_scale_top, (0,0,0), thickness_general + 2, line_type)
    cv2.putText(frame_corregido, "Codigo:", (x_code_label, y_code), font, font_scale_top, font_color_yellow, thickness_general, line_type)
    cv2.putText(frame_corregido, script_name, (x_code_value, y_code), font, font_scale_top, (0,0,0), thickness_general + 2, line_type)
    cv2.putText(frame_corregido, script_name, (x_code_value, y_code), font, font_scale_top, font_color_general, thickness_general, line_type)

    # 4. Fecha Análisis (izquierda sobre ROI)
    ...
    x_date_label = zona_x_inicio
    x_date_value = x_date_label + text_w_date_label + 5
    ...
    # 5. Caja de Información Inferior Izquierda (a, b, c, d)
    box_abcd_x = 20
    box_abcd_y_start = frame_height - 120
    line_height_abcd = 26

    # Amarillo para etiquetas, blanco para valores
    ...
    for i, (label, value) in enumerate(zip(abcd_labels, abcd_values)):
        ...
        (text_w_label, _), _ = cv2.getTextSize(label, font, font_scale_info_boxes, thickness_general)
        (text_w_value, _), _ = cv2.getTextSize(str(value), font, font_scale_info_boxes, thickness_general)
        x_label = box_abcd_x
        x_value = x_label + text_w_label + 5
        cv2.putText(frame_corregido, label, (x_label, y_abcd), font, font_scale_info_boxes, (0,0,0), thickness_general + 2, line_type)
        cv2.putText(frame_corregido, label, (x_label, y_abcd), font, font_scale_info_boxes, font_color_yellow, thickness_general, line_type)
        cv2.putText(frame_corregido, str(value), (x_value, y_abcd), font, font_scale_info_boxes, (0,0,0), thickness_general + 2, line_type)
        cv2.putText(frame_corregido, str(value), (x_value, y_abcd), font, font_scale_info_boxes, font_color_general, thickness_general, line_type)

    # 6. Información Inferior Central (frame, tiempo)
    frame_label = "Frame:"
    time_label = "Tiempo:"
    frame_time_text = f"{frame_label} {frame_idx} {time_label} {tiempo_actual:.1f}s"
    ...
    frame_time_x = box_abcd_x + max(text_w_label + text_w_value for text_w_label, text_w_value in [
        cv2.getTextSize(lbl, font, font_scale_info_boxes, thickness_general)[0] for lbl in abcd_labels
    ]) + 40
    frame_time_y = frame_height - 30

    # Frame label amarillo, valor blanco; Tiempo label amarillo, valor blanco
    x_frame_label = frame_time_x
    x_frame_value = x_frame_label + text_w_frame_label + 5
    x_time_label = x_frame_value + text_w_frame_value + 20
    x_time_value = x_time_label + text_w_time_label + 5
    ...
    # --- START OF ADDED CODE TO DEFINE VARIABLES FOR "ID Reciente" BOX (box_e) ---
    # Define parameters for the "ID Reciente" display box (box_e)
    # These variables were missing, causing a NameError.
    box_e_margin_right = 20  # Margin from the right edge of the frame
    box_e_item_height = 26   # Height for each item/line, similar to line_height_abcd
    # Start Y-coordinate for the first item in this list.
    # This aligns its vertical start with box_abcd (which starts at frame_height - 120)
    # and lists items downwards.
    box_e_y_start = frame_height - 120
    font_color_e = font_color_general  # Use one of the predefined colors (e.g., white)
    # --- END OF ADDED CODE ---

    # Loop to display the last 5 confirmed IDs (where the error originally occurred)
    for i, confirmed_info in enumerate(reversed(ultimas_5_ids_confirmadas)):
        if gps_posiciones_vehiculo:
            a = gps_posiciones_vehiculo[-1][1]  # Longitud
            b = gps_posiciones_vehiculo[-1][0]  # Latitud
        else:
            a, b = 0.0, 0.0

        # Adjusted f-string for GPS coordinates 'a' and 'b' to use 2 decimal places
        text_to_display_e = f"ID Reciente: {confirmed_info['id']},PS x:{confirmed_info['cx']}, y:{confirmed_info['cy']}; PR X{a:.2f}, Y{b:.2f}"
        current_y_e = box_e_y_start + i * box_e_item_height # Now box_e_y_start and box_e_item_height are defined
        (text_w_e, _), _ = cv2.getTextSize(text_to_display_e, font, font_scale_info_boxes, thickness_general)
        actual_text_x_e = frame_width - text_w_e - box_e_margin_right # Now box_e_margin_right is defined
        
        # This condition ensures text is drawn within reasonable vertical bounds
        if current_y_e < frame_height - 5 and current_y_e > box_e_item_height :
             cv2.putText(frame_corregido, text_to_display_e, (actual_text_x_e, current_y_e),
                        font, font_scale_info_boxes, font_color_e, thickness_general, line_type) # Now font_color_e is defined
