...
class TachaTracker:
    ...
    def __init__(self, tacha_id, centro_img, centro_transformado, frame_idx):
        ...
        # Dentro de TachaTracker.__init__
        self.kf.processNoiseCov = np.eye(4, dtype=np.float32) * 0.25 # Ejemplo, prueba diferentes valores

        # Ruido de la Medida (R)
        self.kf.measurementNoiseCov = np.eye(2, dtype=np.float32) * 0.45
...
while cap.isOpened():
    ...
    if active_trackers and detecciones_actuales:
        ...
        matched_detection_indices_in_actuales = list(set(temp_matched_indices)) # Ensure unique indices
    ...
    if unmatched_detections:
        ...
        if missed_confirmed_tracker_indices and unmatched_detections:
            ...
            for i, tracker_global_idx in enumerate(missed_confirmed_tracker_indices):
                ...
                # Use the PREDICTED state (statePre) for re-association cost calculation
                pred_x, pred_y = tracker.kf.statePre[0], tracker.kf.statePre[1]
            ...
            for r_local_idx, c_local_idx in zip(reassoc_row_ind, reassoc_col_ind):
                # r_local_idx: index into missed_confirmed_tracker_indices
                # c_local_idx: index into unmatched_detections
                ...
                if reassoc_cost_matrix[r_local_idx, c_local_idx] < reassoc_dist_threshold:
                    ...
                    # Update the tracker with the re-associated detection
                    centro_img_det = np.array([det_to_reassociate['cx'], det_to_reassociate['cy']])
                    centro_transformado_det = transformar_punto(centro_img_det, M)
                    tracker_to_reassociate.update(centro_img_det, centro_transformado_det) # Resets misses

                    reassociated_detection_indices_in_unmatched.add(c_local_idx)
                    # This tracker is now considered updated, though already handled by not being in the new tracker creation list.
            
            # Collect detections that are STILL unmatched after re-association
            ...
            # No missed confirmed trackers to re-associate OR no unmatched detections for them
            detections_for_new_trackers = list(unmatched_detections)
    else:
        # No unmatched detections from the primary phase
        detections_for_new_trackers = []
 
    # 4b. CREACIÓN DE NUEVOS TRACKERS
    for det in detections_for_new_trackers: # Changed from unmatched_detections
