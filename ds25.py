...
output_video_filename = '3.2-01_04 Tramo B1-B2_resultadoTransformado_Tachav3_GPSmap.MP4'
...
# pts_origen = np.float32([[zona_x_inicio, zona_y_inicio], [zona_x_fin, zona_y_inicio], [zona_x_inicio, zona_y_fin], [zona_x_fin, zona_y_fin]]) # ROI Rectangular
...
script_name = os.path.basename(__file__) if '__file__' in globals() else "beta6_GPS.py" # Nombre actualizado

# --- Variables para el minimapa ---
mini_mapa_h, mini_mapa_w = 200, 200
centro_mapa_x, centro_mapa_y = mini_mapa_w // 2, mini_mapa_h // 2
# escala_mapa_vo se usa si se dibuja la ruta VO. Dejamos su definición por si se descomenta.
escala_mapa_vo = 5 # pixeles por unidad de t_f (asumamos metros para VO)
...
while cap.isOpened():
    ...
    if gps_data is not None and gps_tiempos is not None and gps_trayectoria is not None:
        ...
        if idx_gps < len(gps_trayectoria):
            # Añadir la lat/lon cruda a la lista para el gráfico final de Matplotlib
            gps_posiciones_vehiculo_raw.append(gps_trayectoria[idx_gps])
            
            current_lat, current_lon = gps_trayectoria[idx_gps]

            if gps_origin_lat_lon is None: # Primer punto GPS
                gps_origin_lat_lon = (current_lat, current_lon)
                # Calcular METERS_PER_DEGREE_LON una vez, en la latitud del origen
                ...
                if METERS_PER_DEGREE_LON_AT_ORIGIN is not None:
                    ...
                    dy_meters = delta_lat * METERS_PER_DEGREE_LAT # dy_meters positivo es Norte

                    map_x = centro_mapa_x + int(dx_meters * ESCALA_MAPA_GPS)
                    # Positivo dy_meters (Norte) se mapea hacia abajo en el minimapa
                    # para ser consistente con cómo t_f[2] (adelante en VO) se mapea.
    ...
    for tracker in active_trackers:
        if tracker.estado == 'Confirmado':
            ...
            cv2.putText(frame_corregido, f"ID:{id_text_to_show}", (center_x_img + 10, center_y_img - 10),
    ...
    if prev_gray is not None and prev_des is not None and des is not None and len(kp) > 0:
        try:
            ...
            if len(matches) > 20:
                ...
                if E is not None:
                    _, R_vo, t_vo, mask_rp = cv2.recoverPose(E, dst_pts, src_pts, K) # Renombrado R, t a R_vo, t_vo
                    escala_asumida = 1.0 # La VO es a escala relativa
    ...
    # Ajustar las etiquetas de la grilla para que usen ESCALA_MAPA_GPS si está disponible
    # Si ESCALA_MAPA_GPS es pixels/metro, entonces meters_per_grid = grid_spacing_pixels / ESCALA_MAPA_GPS
    ref_scale_for_grid_text = ESCALA_MAPA_GPS if ESCALA_MAPA_GPS > 0 else escala_mapa_vo # Fallback a escala_mapa_vo si GPS no está bien escalado
    if ref_scale_for_grid_text <= 0: ref_scale_for_grid_text = 1.0 # Evitar división por cero
    ...
    pos_text_vo = f"Posicion VO: X: {t_f[0][0]:.1f}, Y: {t_f[2][0]:.1f}" # Renombrado Y a Z para VO
    ...
    value_text_vo = f"X:{t_f[0][0]:.1f} Z:{t_f[2][0]:.1f}" # Usar Z para VO
    ...
    y_code_f = y_pos_f + text_h_code_label + line_spacing_f # Usar altura de texto de GPS/VO
    ...
    for i, (label, value) in enumerate(zip(abcd_labels, abcd_values)):
        ...
        (text_w_label_abcd, _), _ = cv2.getTextSize(label, font, font_scale_info_boxes, thickness_general) # Renombrado
        x_label_abcd = box_abcd_x; x_value_abcd = x_label_abcd + text_w_label_abcd + 5 # Renombrado
    ...
    # Calcular el ancho máximo de las etiquetas ABCD para alinear "Frame:" y "Tiempo:"
    ...
    font_color_e = font_color_general
    for i, confirmed_info in enumerate(reversed(ultimas_5_ids_confirmadas)):
        ...
        text_to_display_e = f"ID Reciente: {confirmed_info['id']},PS x:{confirmed_info['cx']}, y:{confirmed_info['cy']}; PR Lon:{a_gps:.2f}, Lat:{b_gps:.2f}"
...
if posiciones_vehiculo and gps_posiciones_vehiculo_raw: # Usar gps_posiciones_vehiculo_raw
    odo_np = np.array(posiciones_vehiculo)
    gps_np_raw = np.array(gps_posiciones_vehiculo_raw) # [Lat, Lon]

    plt.figure(figsize=(12, 10))
    
    # Graficar Odometría Visual (escala relativa, origen (0,0))
    plt.plot(odo_np[:, 0], odo_np[:, 1], 'g-', label='Odometría Visual (X, Z relativos)')

    # Graficar GPS (Longitud, Latitud) - Puede necesitar ajuste de escala/origen para superponer bien
    # Para una comparación visual simple, graficamos Lon vs Lat directamente.
    # Para una superposición más precisa, se necesitaría convertir GPS a un sistema local XY en metros
    # y posiblemente escalar/rotar una de las trayectorias.
    if gps_np_raw.ndim == 2 and gps_np_raw.shape[1] == 2:
        # Graficamos Lon (como X) y Lat (como Y)
        # Para que se parezca más a la orientación del minimapa (Este=X, Norte=Y(invertido))
        # y VO (X_vo, Z_vo donde Z es adelante/vertical en minimapa)
        # podrías graficar (Lon - Lon_inicial) vs (Lat_inicial - Lat) para alinear visualmente
        # si Lat aumenta hacia arriba y Lon hacia la derecha.
        
        # Opción 1: Plot directo Lon, Lat
        # plt.plot(gps_np_raw[:, 1], gps_np_raw[:, 0], 'b--', label='GPS (Longitud, Latitud)')
        
        # Opción 2: Graficar desplazamientos en grados (más comparable a la forma de VO si no se escala)
        ...
        # El factor 1e5 es solo para que los números no sean tan pequeños, no es una conversión a metros real aquí.
    ...
    plt.legend()
    plt.grid(True)
    plt.axis('equal') # Importante para ver la forma real de la trayectoria
    try:
