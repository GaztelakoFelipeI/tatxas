...
def registrar_detecciones_nuevas_tiempo(detecciones_actuales_img, historial_obj_tiempo, frame_idx_actual, umbral_dist=30):
    """
    Registra detecciones que son "nuevas" en comparación con un historial reciente.
    Opera con coordenadas de imagen.
    'detecciones_actuales_img' es una lista de diccionarios con {'cx': ..., 'cy': ...}
    'historial_obj_tiempo' es una lista de diccionarios con {'centro_img': (cx, cy), 'frame_idx': ...}
    Devuelve una lista de las detecciones consideradas nuevas.
    """
    nuevas_detectadas_en_este_frame = []
    if not detecciones_actuales_img:
        return nuevas_detectadas_en_este_frame

    # Extraer solo los centros del historial para la comparación de distancia
    centros_historial_img = [h['centro_img'] for h in historial_obj_tiempo]

    for det_actual in detecciones_actuales_img:
        cx_actual, cy_actual = det_actual['cx'], det_actual['cy']
        punto_actual_img = np.array([cx_actual, cy_actual])
        es_genuinamente_nueva = True

        if centros_historial_img:
            for centro_hist_img in centros_historial_img:
                dist = np.linalg.norm(punto_actual_img - np.array(centro_hist_img))
                if dist < umbral_dist:
                    es_genuinamente_nueva = False
                    break
        
        if es_genuinamente_nueva:
            nuevas_detectadas_en_este_frame.append({
                'centro_img': (cx_actual, cy_actual), # Guardamos el centro en coordenadas de imagen
                'frame_idx': frame_idx_actual
                # No se transforma a BEV aquí, esta lógica opera sobre la aparición en imagen
            })
    return nuevas_detectadas_en_este_frame

# Video
ruta_video = 'videos/3.2 - 01_04 Tramo B1-B2.MP4'
output_video = '3.2 - 01_04 Tramo B1-B2_resultadoTransformado_conAnalisisTachas_NN_RealTimeAlert_v2.MP4' # Modificado nombre salida
...
min_frames_entre_detecciones_tiempo = int(fps / 3) if fps > 0 else 10
...
objeto_historial_tiempo = [] # Historial de detecciones para la lógica de tiempo
historial_max_len_tiempo = 50 # Máximo de objetos en el historial de tiempo (para la función registrar_detecciones_nuevas_tiempo)
detecciones_frames_tiempo = [] # Frames donde se registraron detecciones "nuevas" para esta lógica
min_frames_entre_detecciones_tiempo = 10 # Mínimo de frames entre "eventos de detección" (ajustar según FPS)
ventana_promedio_tiempo = 5        # Número de intervalos recientes para calcular el promedio y std dev
promedio_intervalo_frames_tiempo = None
desviacion_intervalo_frames_tiempo = 0.0
tolerancia_std_dev_tiempo = 2.0    # Cuántas desviaciones estándar por encima del promedio para alertar
umbral_dist_nueva_registro_tiempo = 40 # Píxeles para considerar una detección como "nueva" en registrar_detecciones_nuevas_tiempo
...
while cap.isOpened():
    ...
    nuevas_para_logica_tiempo = registrar_detecciones_nuevas_tiempo(
        detecciones_actuales, # Estas son las detecciones filtradas en coords de imagen
        objeto_historial_tiempo, 
        frame_idx,
        umbral_dist=umbral_dist_nueva_registro_tiempo
    )

    if nuevas_para_logica_tiempo: # Si se detectó algo considerado "nuevo" para esta lógica
        # Actualizar el historial de objetos para esta lógica de tiempo
        objeto_historial_tiempo.extend(nuevas_para_logica_tiempo)
        objeto_historial_tiempo = objeto_historial_tiempo[-historial_max_len_tiempo:]

        # Registrar el frame si ha pasado suficiente tiempo desde el último "evento de detección"
        if not detecciones_frames_tiempo or \
           (frame_idx - detecciones_frames_tiempo[-1]) >= min_frames_entre_detecciones_tiempo:
            
            detecciones_frames_tiempo.append(frame_idx)

            # Si tenemos suficientes "eventos de detección" para calcular promedio y std dev
            if len(detecciones_frames_tiempo) >= ventana_promedio_tiempo + 1:
                intervalos_entre_detecciones = [
                    detecciones_frames_tiempo[i] - detecciones_frames_tiempo[i-1] 
                    for i in range(1, len(detecciones_frames_tiempo))
                ]
                
                intervalos_recientes = intervalos_entre_detecciones[-ventana_promedio_tiempo:]
                
                if intervalos_recientes:
                    promedio_intervalo_frames_tiempo = sum(intervalos_recientes) / len(intervalos_recientes)
                    if len(intervalos_recientes) > 1:
                        desviacion_intervalo_frames_tiempo = statistics.stdev(intervalos_recientes)
                    else:
                        desviacion_intervalo_frames_tiempo = 0.0 # O un valor por defecto pequeño
                    # print(f"[DEBUG TIEMPO] Frame: {frame_idx}, Promedio Intervalo: {promedio_intervalo_frames_tiempo:.2f}, Desv: {desviacion_intervalo_frames_tiempo:.2f}")

    # Comprobar si se debe generar una alerta de "FALTA TACHA" por tiempo
    if promedio_intervalo_frames_tiempo is not None and detecciones_frames_tiempo and \
       len(detecciones_frames_tiempo) > ventana_promedio_tiempo : # Asegurarse que el promedio es válido

        tiempo_desde_ultima_deteccion_registrada = frame_idx - detecciones_frames_tiempo[-1]
        
        umbral_alerta_tiempo = promedio_intervalo_frames_tiempo + \
                               (tolerancia_std_dev_tiempo * desviacion_intervalo_frames_tiempo)
        
        # Añadir un umbral mínimo absoluto para evitar alertas si el promedio es muy bajo
        umbral_alerta_tiempo_min_absoluto = min_frames_entre_detecciones_tiempo * 2 # Ejemplo
        umbral_alerta_final = max(umbral_alerta_tiempo, umbral_alerta_tiempo_min_absoluto)


        if tiempo_desde_ultima_deteccion_registrada > umbral_alerta_final :
            # print(f"[ALERTA TIEMPO] Posible falta de tacha en frame {frame_idx}. Tiempo desde última: {tiempo_desde_ultima_deteccion_registrada}, Umbral: {umbral_alerta_final:.2f}")
            cv2.putText(frame_corregido, "FALTA TACHA (Intervalo Anormal)", (frame_width // 2 - 150, 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 165, 255), 2) # Naranja para diferenciarla
    ...
    if active_trackers and detecciones_actuales:
        ...
        for t, tracker_obj in enumerate(active_trackers):
        ...
        for r, c in zip(row_ind, col_ind):
            if cost_matrix[r, c] < max_dist_threshold:
                ...
                tracker_obj.update(centro_img_det, centro_transformado_det)
    ...
    final_trackers = []
    for tracker_obj in active_trackers: 
        if tracker_obj.estado == 'Tentativo' and tracker_obj.hits >= MIN_HITS_TO_CONFIRM:
            ...
            if tracker_obj.display_id not in ultimas_5_ids_confirmadas : 
    ...
    if len(current_confirmed_active_studs) >= MIN_STUDS_FOR_REALTIME_PATTERN:
        ...
        if spacings_current_active:
            # SUGERENCIA: Considerar usar np.median(spacings_current_active) para mayor robustez
        ...
        for i in range(len(current_confirmed_active_studs) - 1):
            ...
            if dist_entre_detectadas_bev > (effective_expected_spacing_bev * REALTIME_SPACING_TOLERANCE_FACTOR):
                ...
                if num_faltantes_estimadas > 0:
                    ...
                    for k_idx in range(1, num_faltantes_estimadas + 1): 
                        ...
                        # --- INICIO: DIBUJAR ALERTA INDIVIDUAL DE TACHA FALTANTE ---
                        pos_img_faltante = transformar_punto_inv(pos_faltante_estimada_bev, M_inv)
                        if pos_img_faltante[0] > 0 and pos_img_faltante[1] > 0 and \
                           0 <= pos_img_faltante[0] < frame_width and 0 <= pos_img_faltante[1] < frame_height : # Chequear límites
                            cv2.circle(frame_corregido, (int(pos_img_faltante[0]), int(pos_img_faltante[1])), 8, (0, 0, 255), 2) # Círculo rojo
                            cv2.putText(frame_corregido, "F?", (int(pos_img_faltante[0]) + 10, int(pos_img_faltante[1]) - 10),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                        # --- FIN: DIBUJAR ALERTA INDIVIDUAL DE TACHA FALTANTE ---
    ...
    if active_missing_stud_notifications:
        ...
        cv2.rectangle(frame_corregido, (5, 5), (10 + text_w + 5, 5 + text_h + baseline + 5), (0,0,0), -1) # Fondo negro
        cv2.putText(frame_corregido, alert_text,
                    (10, 5 + text_h + 5), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA) # Texto rojo
    ...
    if prev_gray is not None and prev_des is not None and des is not None and len(kp) > 0:
        try:
            ...
            if len(matches) > 20:
                ...
                if E is not None:
                    _, R_cv, t_cv, mask_rp = cv2.recoverPose(E, dst_pts, src_pts, K) 
                    escala_asumida = 1.0
    ...
    if len(posiciones_vehiculo) > 1:
        for i_mapa in range(1, len(posiciones_vehiculo)): 
...
if len(historial_tachas_ordenado) > 1:
    for i_dist in range(1, len(historial_tachas_ordenado)): 
...
if distancias_entre_tachas_detectadas_list:
    # SUGERENCIA: Considerar usar np.median para mayor robustez contra outliers
    # media_espaciado_pix_bev = np.median(distancias_entre_tachas_detectadas_list)
...
nombre_archivo_excel = f"{fecha_actual_str}_Analisis_Completo_Tachas_vRT_v2.xlsx" # Modificado nombre
...
if posiciones_vehiculo and (gps_data is not None and len(gps_posiciones_vehiculo) > 0):
    ...
    plt.axis('equal')
    try:
        plt.savefig('trayectoria_comparativa_vo_gps_vRT_v2.png', dpi=300) # Modificado nombre
