...
while cap.isOpened():
    ...
    tray = np.zeros((500, 500, 3), dtype=np.uint8)
    for i in range(1, len(posiciones_vehiculo)):
        p1 = (int(posiciones_vehiculo[i-1][0]*10 + 250), int(posiciones_vehiculo[i-1][1]*10 + 250))
        p2 = (int(posiciones_vehiculo[i][0]*10 + 250), int(posiciones_vehiculo[i][1]*10 + 250))
        cv2.line(tray, p1, p2, (0, 255, 0), 2)
    cv2.imshow('Trayectoria', tray)
...
import matplotlib.pyplot as plt
posiciones = np.array(posiciones_vehiculo)
plt.plot(posiciones[:, 0], posiciones[:, 1], 'g-')
