...
# Trayectoria GPS
gps_data = pd.read_excel("3.2 - 01_04 Tramo B1-B2.xlsx")
...
# #Generación de mapa GPS (intento número 2)
def generar_mapa_ruta_dinamico_html(gps_coords_recorridos, punto_actual, idx_frame):
    ...
    html_path = f'mapas_interactivos/mapa_frame_{idx_frame:05d}.html'
    m.save(html_path)

    # (Opcional) Abrir automáticamente el HTML en el navegador
    if idx_frame % 500 == 0:  # Para no abrir muchos
    ...
    if len(non_blacks_rows) > 0:
        ...
        print(f"[MAPA INT.] Mapa generado en {screenshot_path}")
...
# Video
ruta_video = '3.2 - 01_04 Tramo B1-B2.MP4'
...
while cap.isOpened():
    ...
    escala_mapa = 20
    mini_mapa = cv2.imread('mapa_satelital.png')
    if mini_mapa is not None:
        ...
        if frame_idx % 100 == 0 and len(gps_posiciones_vehiculo) > 1:
            ...
            print("Primeros puntos GPS:", gps_coords_recorridos[:5])
