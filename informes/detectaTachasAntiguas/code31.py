...
# Se ajusta el código original con una estimación más estable y análisis de patrón de tachas
...
# Historial de detecciones por clase
historial_detecciones_clase = {'tacha': []}
...
output_video = '3.2 - 01_04 Tramo B1-B2_resultadoTransformado_AnalisisTachas.MP4'
...
while cap.isOpened():
    ...
    captafaros, tachas_deteccion, senaleticas = detectar_objetos(frame)
    objetos_filtrados = filtrar_detecciones(tachas_deteccion, captafaros, senaleticas,
    ...
    # Actualizar historial de detecciones de tachas
    for centro in objetos_filtrados:
        transformado = transformar_punto(centro, M)
        historial_detecciones_clase['tacha'].append({'centro': transformado, 'frame': frame_idx})

    # Calcular distancias entre tachas consecutivas (en el espacio transformado)
    if len(historial_detecciones_clase['tacha']) > 1:
        for i in range(len(historial_detecciones_clase['tacha']) - 1):
            p1 = historial_detecciones_clase['tacha'][i]['centro']
            p2 = historial_detecciones_clase['tacha'][i+1]['centro']
            distancia = np.linalg.norm(p2 - p1)
            # Considerar solo distancias entre detecciones no muy separadas en tiempo
            if historial_detecciones_clase['tacha'][i+1]['frame'] - historial_detecciones_clase['tacha'][i]['frame'] < fps * 2:
...
plt.grid(True)
plt.axis('equal')
plt.savefig('trayectoria_comparativa.png', dpi=300)
plt.show()

# Análisis del patrón de colocación de tachas
if distancias_entre_tachas:
    media_distancia = statistics.mean(distancias_entre_tachas)
    desviacion_estandar_distancia = statistics.stdev(distancias_entre_tachas)
    print("\n--- Análisis del Patrón de Colocación de Tachas ---")
    print(f"Número de distancias calculadas: {len(distancias_entre_tachas)}")
    print(f"Media de la distancia entre tachas consecutivas (en el espacio transformado): {media_distancia:.2f} píxeles")
    print(f"Desviación estándar de la distancia entre tachas consecutivas: {desviacion_estandar_distancia:.2f} píxeles")

    plt.figure(figsize=(8, 5))
    plt.hist(distancias_entre_tachas, bins=20, color='orange', edgecolor='black')
    plt.title('Distribución de Distancias entre Tachas Consecutivas')
    plt.xlabel('Distancia (píxeles en el espacio transformado)')
    plt.ylabel('Frecuencia')
    plt.grid(axis='y', alpha=0.75)
    plt.savefig('distribucion_distancias_tachas.png', dpi=300)
    plt.show()
else:
    print("\nNo se detectaron suficientes tachas consecutivas para analizar el patrón de colocación.")
