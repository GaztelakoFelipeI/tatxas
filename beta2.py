...
def filtrar_detecciones(objetos, otros1, otros2, x0, x1, y0, y1, current_frame_idx, min_dist_entre_tachas):
    filtrados_detailed = []
    for obj in objetos:
        ...
        demasiado_cerca_tacha = False # Initialize the variable here
        for f_det in filtrados_detailed:
            if np.linalg.norm([cx - f_det['cx'], cy - f_det['cy']]) < min_dist_entre_tachas:
                demasiado_cerca_tacha = True
                break
        if demasiado_cerca_tacha: continue # This line needs to be inside the loop for each `obj`
...
while cap.isOpened():
    ...
    detecciones_actuales = filtrar_detecciones(
        ...
        frame_idx,
        MIN_DIST_ENTRE_TACHAS_PX
    )

    #Nueva Logica para Tacha Falta
